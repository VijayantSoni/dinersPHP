<?php if(!$cartItem): ?>
	<?php $cart = ShoppingCart::model()->findByAttributes(array('customer_id'=>Yii::app()->user->id)); ?>
	<?php $cartItemArray = ShoppingCartHasItems::model()->findAllByAttributes(array('shopping_cart_id'=>$cart->id,'status'=>1)); ?>
<?php endif; ?>
<?php
	$sum = 0;
	$cartIds = array();
	foreach ($cartItemArray as $cartItem) {
		$sum += $cartItem->item_cost;
		...
		<form class="row" id="order-confirmation">
			...
			<div class="order-details">
				<div class="row">
					...
					<div class="colGLG-6">
						<input id="amount" type="text" disabled="true" value='<?php echo $sum." INR" ?>'>
					</div>
				</div>
			</div>
			<div class="select row">
